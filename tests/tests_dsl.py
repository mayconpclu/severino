...
def replace(import_path: str, mock_type: Optional[type[object]], custom_assert: Optional[Callable[[Mock], None]] = None) -> Callable[[Callable[..., None]], Callable[..., None]]:
    """
    Decorator que utiliza `unittest.mock.patch` para substituir o import passado no argumento `import_path` por um objeto do tipo passado no argumento `mock_type`. A instância substituta é injetada nos `*args` da função.\n
    Opcionalmente, o callback `custom_assert` pode ser utilizado para realizar validações da própria chamada do import substituido.
    """
    def decorator(function: Callable[..., None]) -> Callable[..., None]:
        def wrapper(*args: Any, **kwargs: Any) -> None:
